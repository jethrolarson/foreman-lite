...
- SHOULD: emit `verifier_signal` only when you actually reached a verdict, not to clear the turn-end reminder. CONTEXT: the reminder fires on any signal-less turn, including one where you only answered a question from a human attached to your pane. REASON: every verdict routes to Foreman as Task Thread state, so one sent only to satisfy the reminder is noise Foreman has to triage.
