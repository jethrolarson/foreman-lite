...
- SHOULD: emit `worker_signal` only when task lifecycle actually moved (a plan got confirmed, work became ready, you got blocked), not to clear the turn-end reminder. CONTEXT: the reminder fires on any signal-less turn, including one where you only answered a question from a human attached to your pane. REASON: every signal routes to Foreman as Task Thread state, so one sent only to satisfy the reminder is noise Foreman has to triage.
